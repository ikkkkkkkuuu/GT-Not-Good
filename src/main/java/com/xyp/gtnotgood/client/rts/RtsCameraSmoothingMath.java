...
final class RtsCameraSmoothingMath {
    ...
    static float approachAxis(float current, float target, float deltaSeconds, float accelerationSeconds,
        float decelerationSeconds, float epsilon) {
        ...
        boolean reversing = current != 0.0F && target != 0.0F && Math.signum(current) != Math.signum(target);
        boolean slowingDown = Math.abs(target) < Math.abs(current);
        float responseSeconds = reversing || slowingDown ? decelerationSeconds : accelerationSeconds;
    ...
    static DecayStep consumeRemaining(float remaining, float deltaSeconds, float responseSeconds, float epsilon) {
    ...
    /** Immutable DecayStep value; Java 8 replacement for the upstream record. */
