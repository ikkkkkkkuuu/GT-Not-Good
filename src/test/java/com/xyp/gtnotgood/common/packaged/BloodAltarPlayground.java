...
@Mod(
    modid = "bloodplayground",
    name = "Blood Altar Playground",
    version = "1",
...
public final class BloodAltarPlayground {
    ...
    public void init(FMLInitializationEvent event) {
        if (Boolean.getBoolean("gtng.blood.playground")) FMLCommonHandler.instance()
            .bus()
            .register(this);
    }
    ...
    public void client(TickEvent.ClientTickEvent event) throws Exception {
        ...
        if (!launched && mc.theWorld == null && mc.currentScreen != null) {
            ...
            if (resumed && !save.matches("blood-manual-[0-9]+"))
    ...
    private void build(World world, EntityPlayerMP player) {
        ...
        for (var component : UpgradedAltars.getAltarUpgradeListForTier(5)) {
            world.setBlock(
                ...
                component.getMetadata() == 32767 ? 0 : component.getMetadata(),
                3);
        }
        ...
        ((TileDrive) world.getTileEntity(-2, 8, -4)).getInternalInventory()
        ...
        for (int slot = 0; slot < 5; slot++) {
            ...
            System.out
                .println("BLOOD_PLAYGROUND_RECIPE: " + ingredient.getDisplayName() + " -> " + output.getDisplayName());
    ...
    private static ItemStack pattern(ItemStack input, ItemStack output) {
        ItemStack stack = AEApi.instance()
            .definitions()
            .items()
            .encodedPattern()
            .maybeStack(1)
